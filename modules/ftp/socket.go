...
func newActiveSocket(remote string, port int, sessionID string) (DataSocket, error) {
	...
	log.Println(sessionID, "Opening active data connection to "+connectTo)

	raddr, err := net.ResolveTCPAddr("tcp", connectTo)

	if err != nil {
		log.Println(sessionID, err)
		return nil, err
	}
...
func (conn *Conn) newPassiveSocket() (DataSocket, error) {
	...
	socket.host = conn.conn.LocalAddr().(*net.TCPAddr).IP.String()

	const retries = 10
	var err error
	for i := 1; i <= retries; i++ {
		socket.port = conn.server.settings.MinPassivePort + rand.Intn(conn.server.settings.MaxPassivePort-conn.server.settings.MinPassivePort)
		err = socket.GoListenAndServe(conn.session)
